...
[$image, $img_err] = save_food_image($_FILES['image'] ?? null);
    ...
    if (empty($name) || $price <= 0 || $cid <= 0) {
        ...
        $error = $img_err;
    } else {$s = $conn->prepare("INSERT INTO foods(category_id,name,description,price,image,available) VALUES(?,?,?,?,?,?)");
...
<img src="<?= htmlspecialchars(food_image($f, '../')) ?>" alt="<?= htmlspecialchars($f['name']) ?>"
                                         style="width:50px;height:50px;object-fit:cover;border-radius:6px;border:1px solid var(--border)"></td>
                                <td>
...
onclick="openFoodEdit(<?= $f['id'] ?>, <?= $f['category_id'] ?>, '<?= htmlspecialchars(addslashes($f['name'])) ?>', '<?= htmlspecialchars(addslashes($f['description'])) ?>', <?= $f['price'] ?>, <?= $f['available'] ?>, '<?= htmlspecialchars(addslashes(food_image($f, '../'))) ?>')">✏️ Edit
                                        </button>
...
<script>
function openFoodEdit(id, catId, name, desc, price, avail, imageUrl) {document.getElementById('fe_id').value    = id;
...
imgContainer.innerHTML = '<img src="' + imageUrl + '" alt="Current picture" style="width:100px;height:100px;object-fit:cover;border-radius:8px;border:2px solid var(--border)">';document.getElementById('foodEditModal').style.display = 'flex';
}
</script>
</body>
</html>
